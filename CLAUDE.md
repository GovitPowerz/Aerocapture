...
make -C articles/paper             # Paper figures from the committed bundle (no Rust, no run logs); `paper` = fetch-logs -> results -> confirmatory-marginal -> quote-marginal -> figures -> provenance -> pdf
make -C articles/paper check       # Bundle SHA256SUMS + results.json schema + the two per-scenario extracts (confirmatory_marginal.json, quote_marginal.json) current + figures/results.json unchanged in git (CI runs `-B figures` then this on every PR)
...
updated binary on each test config and replacing the CSV files. The 6 guidance golden files cover: eqglide, energy_ctrl, pred_guid, fnpag, ftc, neural; the three legacy validation goldens (ref, guided, high_bank: final + photo) move with them. Virtual-DV changes affect only the `dv_total_m_s` column
...
26-long (= `n_var`) bounds mask raises `IndexError: boolean index did not match indexed array along axis 1`. `SingleAlgoTrainer.should_continue` therefore ends `run_loop` at the top of every iteration with a CMA-ES-gated
`is_cmaes and not algorithm.has_next()` check (the `is_cmaes` instance check, computed once before the loop, also gates the pre-next re-eval skip), so a converged CMA-ES ends cleanly and the
