...
- GA training pipeline: optimizes any guidance scheme's parameters (not just NN weights)
  - `train.py` — Hybrid pymoo training loop with checkpoint save/resume (`<config.toml> [--no-tui] [--adaptive-seeds] [--seed-pool-cap N] [--cost-alpha F] [--cvar-percentile P] [--skip-report] [--final-n-sims N] [--algorithm ALG]`). Uses pymoo `Algorithm` objects stepped manually via `algorithm.next()` in a custom outer loop. Supports GA (SBX + polynomial mutation), CMA-ES, DE, and PSO via `--algorithm` or `[optimizer] algorithm` in TOML. Auto-resumes from existing checkpoint when output dir exists (no `--resume` needed); `--resume` only needed to specify a non-default directory. On resume, `--n-gen` means "N additional generations" (not total). A checkpoint is always saved at end of training (not just at interval multiples). Graceful KeyboardInterrupt handling: Ctrl+C saves checkpoint and returns cleanly with `interrupted: True`. Epoch seed rotation: when `training_n_sims > 1`, draws fresh random MC seeds each generation so the fitness landscape never stabilizes (prevents overfitting to fixed scenarios); epoch seeds exclude both validation and final-eval reserved seed sets. After the seed rotation, `algorithm.pop` is re-evaluated on the new seeds **before** `algorithm.next()` for GA, DE, and PSO -- these carry F across generations (GA's `FitnessSurvival`, DE's `ImprovementReplacement` on target vs trial, PSO's `pbest` comparison and `_social_best`), and without the re-eval the internal comparison mixes stale prev-seed F with fresh current-seed F (causing the true elite to be dropped in large-parameter schemes like NN/PC where offspring F variance is high). The re-eval is skipped for CMA-ES (detected via `isinstance(algorithm, (CMAES, SimpleCMAES))` to handle the `n_params > 2000` GA fallback), because CMA-ES samples a fresh population from `es.ask()` each generation and does not carry F across gens. Validation gate: triggered by **parameter identity** -- if the current gen's argmin individual differs from `last_validated_individual` (`np.array_equal`), run the validation MC on the reserved seed set (`validation_n_sims`, default 1000). Cost-based new-best detection is unreliable under rotating/adaptive seeds, hence the identity trigger. Validation records include `rms_cost` (the promotion metric), mean, p95, worst, and capture rate; `best_overall_individual` is promoted only when `val_rms < best_val_cost`. The `improvement` flag in logger records reflects **validation promotions**, keeping the TUI's "Stagnant for N gens" counter honest under rotating seeds. TUI shows "Last val" (most recent attempt with PROMOTED/REJECTED outcome) and "Best val" (lowest-RMS validated candidate, permanent). Three reserved seed pools (training, validation, final eval) use well-separated RNG streams via `make_reserved_seeds(base, offset, n)` with offsets `VALIDATION_SEED_OFFSET` (1M) and `FINAL_EVAL_SEED_OFFSET` (2M) to guarantee zero overlap. Adaptive seed pool (via `--adaptive-seeds`) updates on every new gen-best individual (by identity) or a periodic fallback: adds seeds, scores difficulty, evicts redundant, re-evaluates population on updated seeds; bootstrap creates 10 seeds (configurable). At end of training, generates a single PDF report (convergence + final MC evaluation) via `report.py` unless `--skip-report` is passed.
