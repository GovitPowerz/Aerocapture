...
- Training visualization:
  ...
  - `final_report.py` — Post-training final evaluation: runs 1000-sim MC re-evaluation via `run_mc(include_trajectories=True)`, returns `FinalEvalData(final_array, trajectories, dispersions)`. Generates Plotly HTML with delta-V/orbital error distributions, entry/exit conditions scatter, performance summary table (g-load, heat flux, bank consumption, orbital errors, DV with Mean/Std/Min/p5-p95/Max), energy corridor panels (pdyn, inclination, bank angle vs energy with spaghetti + envelope + reference trajectory), and dispersion correlation grid (~24 scatter subplots with linear regression). Auto-generated at end of training, also standalone CLI: `python -m aerocapture.training.final_report`
