...
- GA training pipeline: optimizes any guidance scheme's parameters (not just NN weights)
  ...
  - `seed_curator.py` — Curated-CDF seed framework: `SeedCurator` class that maintains a fixed-size training seed list with quantile-stratified curation. `curate(top_k_individuals, sample_size, n_bins)` draws fresh candidate seeds, evaluates top-k individuals via PyO3 batch, averages per-seed costs, splits into equal-count quantile bins, and picks one seed per bin. `should_curate(gen, interval)` returns True on periodic fallback trigger. Checkpoint serialization via `to_dict()`/`from_dict()`.
