...
## Stateful NN Runtime Infrastructure (Phase 0 + Phase 1 GRU MVP + Phase 1.5 PPO-GRU)
...
**Extensibility (post-Phase-1.5 contract)**: adding a Phase 2+ layer type (LSTM, Window-MLP, Attention, SSM) touches only `neural.rs` (LayerSpec/Layer/XxxLayer/LayerWeights/save_json/from_v2_json/from_flat_weights_v2 arms), `nn_state.rs` (LayerState variant + for_layer + reset arms), `config.rs` (TomlLayerSpec variant + to_layer_spec arm), `rl/layers/<type>.py` (new file), `rl/layers/__init__.py` (dispatch line), `rl/schemas.py` (Spec class + union entry), `encoding.py` (dispatch branch), `rl/export.py` + `model_io.py` (isinstance branches), plus a `_layer_n_params` arm in `config.py`. No further changes to `train.py`, `problem.py`, `ppo.py`, `dispatch.rs`, or `runner.rs`. Multi-tensor hidden states (e.g. LSTM `(h, c)`) require extending `_zero_state_where_done` in `policy.py`; the current helper raises `TypeError` on unsupported state types to force the extension rather than silently matmul-erroring.
