...
`articles/paper/data/*.json`; `appendix.typ` reads `figures/appendix/<scheme>/stats.json` at compile
time (the body's table numbers are transcribed from the data files). `make -C articles/paper paper`
