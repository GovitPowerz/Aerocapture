...
- [ ] 8.4 Objective shaping: cost_transform (fig_cost_transform) + curation bucket/trim (fig_curation). **DECIDED 2026-06-14 (Study D / 05, deployed DV @300): the dual-role tradeoff has an INTERIOR OPTIMUM, and the project default (cubed) is PAST it.** Moments ordered lightest→heaviest tail-weight = log < sqrt < linear < squared < cubed. Deployed CVaR95: log **142.1** (worst — too light, doesn't push hard seeds), sqrt **134.1**, squared 135.4, linear 135.6, cubed 137.6; deployed mean: sqrt 117.3, linear/log 118.0, squared 118.9, cubed **120.4** (worst). So **cubed (max tail-weight) does NOT minimize the deployed tail** — the selection-noise cost (rank-by-worst-of-10 dominated by 1-2 seeds) overtakes the tail benefit; it only wins the descriptive sample max (148.1). The robust, noise-proof claims: (i) log is clearly worst on the tail; (ii) the interior transforms {sqrt, linear, squared} cluster ~117-119 mean / 134-136 CVaR95 and beat BOTH extremes; (iii) maximal tail-weighting is counterproductive. Paired cubed−log: +2.48 m/s mean (cubed worse, p=8e-28) but −4.4 CVaR95 (cubed better than log) — the classic tradeoff, except sqrt dominates both. The dual-role mechanism is thus CONFIRMED and RESOLVED: train on a MILD transform (sqrt) — it is clean enough for selection AND tail-aware enough for sizing; the log-train/tail-select hybrid is moot (sqrt already is the sweet spot). **Implication for the headline:** ga_300 (cubed) is ~3 m/s worse than a sqrt-trained equivalent on BOTH mean and CVaR95; whether to switch the deployed default to sqrt is DEFERRED to after seed-repeats (11) gives σ_run (the sqrt-vs-cubed mean gap ~3 m/s is comparable to the GA cubed @150-vs-@300 run-noise of 2.4 m/s, so the fine sqrt>linear ranking needs σ_run; the "cubed is past the optimum" qualitative finding does not). The campaign stays cubed for cross-study consistency (all optimizer/seed cells are cubed-controlled); Study D is the held-out transform axis.
