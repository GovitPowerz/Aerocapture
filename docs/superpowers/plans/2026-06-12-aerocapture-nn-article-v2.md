...
- [ ] 8.9 Classical vs NN — the **3-way NN / joint-FTC / FNPAG** (fig_classical_vs_nn compute-vs-CVaR99.9 scatter). **RESHAPED by Study E (07):** joint-reference optimization recovered FTC from 170.7 to **126.2 mean / 142.9 CVaR95 (−44 m/s, paired p=3e-165)** — confirming the reference WAS FTC's weakness. So the best classical is now **joint-FTC**, which MATCHES FNPAG (124.3 / 144.0) on accuracy across the whole distribution (jointftc_vs_fnpag tied: FNPAG +2 m/s mean, joint-FTC better CVaR95) AND is ANALYTIC/fast (~50× cheaper than FNPAG's predictor). The NN beats this much stronger bar: **NN−joint-FTC −5.8 m/s mean (76% paired win, p=1e-69)** and, at the sizing depth (far-tail n=10000), NN CVaR99.9 152.9 / max 160.1 vs joint-FTC 164.0/170.5 ≈ FNPAG 165.0/175.5 — **~11 m/s better at the design tail** (the NN compresses the extreme tail better than either classical). Headline: *the NN beats the best classical at the fastest compute class; the margin GROWS at the sizing tail.* Secondary classical story: joint-FTC = FNPAG accuracy at 50× less compute (the reference-design methodology). NN still crushes fixed-FTC/pred_guid (−50/−47). EC/eqglide/piecewise to an appendix table. compute-benchmark (Task 5b) PENDING an idle box; σ_run (11) calibrates the ~6 m/s NN−jointFTC gap.
