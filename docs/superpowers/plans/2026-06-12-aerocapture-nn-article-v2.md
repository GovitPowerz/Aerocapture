...
- [ ] 8.9 Classical vs NN — the **3-way NN/FTC/FNPAG** (fig_classical_vs_nn compute-vs-CVaR95 scatter): the 2026-06 FNPAG density-fix made FNPAG the strongest classical (124.3 mean / 144.0 CVaR95, beating FTC 170.7), so the headline is NOT "NN crushes classical" — NN beats FNPAG by only −3.8 m/s (69.5% paired win, p=4e-54) but at ~50× less onboard compute. Frame as **parity-at-a-fraction-of-the-compute** (predictor-corrector quality distilled into a flight-deployable net). NN still crushes FTC/pred_guid (−50/−47 m/s). + joint-reference progression (fig_joint_reference, after 07 — may lift FTC toward FNPAG). EC/eqglide/piecewise to an appendix table. HELD until 07 + 11 (σ_run) land.
