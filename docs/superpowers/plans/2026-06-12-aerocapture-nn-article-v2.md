...
# NOTE (2026-06-25): the LIVE aggregator (articles/paper/scripts/aggregate_results.py)
# is the source of truth. HEADLINE moved ga_300 -> dense_p515 -> headline/mamba_p962
# (the 10c sizing winner); paired tables also carry MARGINAL TAIL deltas
# (delta_p95 / delta_cvar95) since the headline is a TAIL winner. Snippet below is illustrative.
HEADLINE = "headline/mamba_p962"
# Paired tables: (label, run_a, run_b) -- delta = a - b, negative = a better.
PAIRED = [
    ("ga_vs_islands_300", "optimizer_budget/ga_300", "optimizer_budget/islands_300"),  # ga_300 = Study A cell (NOT the headline)
...
DONE (2026-06-25) on the HEADLINE = **Mamba_962** (ran for dense_515 first, then re-ran on Mamba after the 10c switch). Commands below updated to the headline; the `--model`/scaffolding flags shipped 2026-06-12.

- [x] **Step 1: ablation on the headline (Mamba_962)**

```bash
uv run python -m aerocapture.training.ablation training_output/mamba_p962_long \
    --toml configs/training/sweep/mamba_p962.toml --n-sims 500 --cost-transform log
```
Result: Mamba leans on eccentricity_excess (+3.81) + hdot_nominal (+3.58) + pdyn_error (+2.96) + predicted_dv2/3 (the engineered autoregressive inputs — same as dense; state handles the tail). Writes `ablation_results.json` into the cell dir.

- [x] **Step 2: input behavior report on the same dir**

```bash
uv run python -m aerocapture.training.nn_input_report training_output/mamba_p962_long \
    --toml configs/training/sweep/mamba_p962.toml --n-sims 200 --dv-threshold 110
```

- [ ] **Step 3: `fig_ablation.py`** — horizontal bar of per-input cost delta from `mamba_p962_long/ablation_results.json` (reuse `charts_ablation.chart_ablation_bar`); re-run `12_collect_results.sh` so the JSON lands in the bundle; commit.

---

### Task 5: Fresh-pool headline re-quote *(gate: campaign 02 complete)*

- [x] **Step 1: Run it** (script shipped 2026-06-12; DONE on the Mamba_962 headline 2026-06-25)

```bash
uv run python articles/paper/scripts/fresh_pool_requote.py \
    training_output/mamba_p962_long --toml configs/training/sweep/mamba_p962.toml
```
Result (the abstract number): p50 109.7 / p95 113.8 / **CVaR95 115.2** / p99 116.0, 100% capture — matches the 2M pool (no selection optimism). Writes `fresh_pool_requote.json`.
...
```
**RESULT (5c, high atmo/density/nav, 9M pool, n=2000):** joint-FTC MOST robust (94.5% capture, -5.5) > NN 93.8% > FNPAG 92.4% > PredGuid 90.8% >> FTC-fixed 67.1% (catastrophic). A WELL-REFERENCED FTC is more robust than FNPAG (confirms the hypothesis); FTC-fixed's fragility is the reference (ties Study E -- reference drives FTC accuracy AND robustness). Robustness axis of the triangle: joint-FTC > NN > FNPAG.
...
Old Task 13 CORRECTED: the canonical reference is the **constant-bank target-energy-matched** nominal (`make_reference.py`) — present the reference-design progression as the methodological arc: constant-bank (works; coverage lesson: a reference must reach target energy) → PC-optimized open-loop (tried, under-reaches, refuted) → **jointly-optimized `ref_bank` gene (Study E)**. **Study E RESULT (07): joint reference recovers −44/−35/−23 m/s for FTC/EC/pred_guid** (`joint_vs_fixed_*` paired, all p<1e-110) — the reference was the dominant FTC weakness, exactly as hypothesized. Joint-FTC (126.2 mean) reaches FNPAG-level accuracy (124.3) while staying analytic/fast (~50× cheaper than FNPAG's forward predictor) — the section's punchline: a co-optimized reference makes the simplest analytic feedback law competitive with the numerical predictor-corrector. Keep FTC Eq. 10 + roll-reversal as-is. Cite cerimele1985, cherry1964, Lu (FNPAG).
...
REWRITTEN from old Task 15, and REFRAMED 2026-06-14 by the Study C result: the centerpiece is **the adaptive-seed methodology as the load-bearing contribution** — the quartet (GA + non-stationary seeds + cubed + max) is a **matched SYSTEM, not four independent wins**. The mechanism, demonstrated by Study C (04): GA does NOT win because it is intrinsically robust to a moving objective — under FIXED (stationary) seeds GA is the WORST optimizer (160.3 deployed, overfitting the 10 repeated training scenarios); non-stationary seeds RESCUE it (fixed→rotating→adaptive = 160.3→120.0→118.0, a 42 m/s swing), while CMA-ES is flat (~127). So **GA *needs* the moving environment** — population recombination over-exploits a stationary objective but exploits the moving objective's implicit regularization best; the adaptive-seed strategy is what converts GA from worst optimizer to best. The iso-compute clincher: GA rotating (4.96M sims) vs GA fixed (4.35M) is near-iso-compute (1.14×) yet +40 m/s; CMA-ES fixed vs rotating is EXACTLY iso-compute (5.00M both) and +0 — so the effect is the seed strategy, not compute (the budget axis showed 2× compute buys ~0). Legs 3/4 (cubed/max) are motivated by the propellant-sizing argument (the tail IS the mission cost function). Optimizer lineage GA(2009) → QPSO(2015 speech) → GA again, vindicated AT HIGH DIMENSION ONLY (see 8.2). Protocol: 6 optimizers × 3 budgets × 3 dimensionalities (26/515/3998), evals/gen matched, **actual sims reported per cell** (from `results.json["runs"][k]["actual_sims"]`); CMA-ES popsize footnote (evals-matched, not CMA-canonical). Warm-start + RL presented as budget-noted negative results (legacy regime footnote).
...
- [ ] 8.1 Optimizer × budget (fig_optimizer A+C): GA best at @150/@300 BUT **collapses at @60** (166.3 — n_pop=60 starves the 4000-dim search, 3× training RMS, while islands/PSO/QPSO hold ~120-124). "GA dominates at every budget" is REFUTED; the honest reading is "GA is the strongest population method *given adequate population*, but uniquely population-hungry." islands is the most budget-robust. actual-sims column. (GA@60 is N=1 — one repeat would confirm the collapse is structural; flag as optional in 11.)
- [ ] 8.2 Optimizer × dimensionality (fig_optimizer B): the ranking does NOT flip — at **26p (FTC gains) ALL optimizers tie (~170, GA actually worst on tail, CVaR95 244 vs islands 229)**; the "CMA-ES sweet spot at low dim" hypothesis is REFUTED (171.6, mid-pack). GA separates ONLY at high dim (best at 515 & 3998). State the confound plainly: 26p is FTC (a different scheme/landscape), so this is "smooth low-dim classical-gain tuning" vs "rugged high-dim NN weights", not a pure width sweep. Practical takeaway: **optimizer choice is irrelevant for classical-gain tuning, decisive for NN weights.**
- [ ] 8.3 Seed strategy (fig_seed_strategy — THE thesis figure, the campaign's biggest single effect): GA fixed→rotating→adaptive = 160.3→120.0→118.0 (−42 m/s), CMA-ES flat (~127), islands like GA (145→120), PSO modest (140→130). Lead with the iso-compute clincher (GA rotating-vs-fixed +40 at 1.14× compute; CMA-ES +0 at exact iso-compute). Reading: **GA is the optimizer that benefits MOST from non-stationary seeds because it overfits a stationary objective hardest** — under fixed seeds GA is the *worst*, adaptive seeds make it the *best*. This is the matched-system thesis, demonstrated not asserted.
- [ ] 8.4 Objective shaping: cost_transform (fig_cost_transform) + curation bucket/trim (fig_curation). **DECIDED 2026-06-14 (Study D / 05), with the FAR-TAIL correction (n=10000, the `far_tail_eval.py` re-eval): the optimal cost_transform DEPENDS ON THE MISSION SIZING PERCENTILE — this coupling IS the result, not a single "best transform".** Moments lightest→heaviest tail-weight = log < sqrt < linear < squared < cubed. At a SHALLOW tail (p95–CVaR95, n=1000) the mild transforms edge cubed (sqrt CVaR95 134.1 vs cubed 137.6) and cubed reads as "past the optimum" — BUT propellant sizing uses the FAR tail (3σ ≈ p99.87 / CVaR99.9 / worst-case), and there the ranking FLIPS (n=10000): at p99/CVaR99 the transforms TIE (~143-145), but at **CVaR99.9 cubed wins (153.0 vs linear 156.7, sqrt 158.4, squared 162.7, log 162.3)** and at the **worst-case (max≈p99.99) cubed wins (160.1 vs 162-181)**. Mechanism: cubed COMPRESSES the extreme tail (p99→max span 19 m/s vs sqrt 30, squared 44 — squared is bimodal, great at p99 / terrible at the extreme). So **the cubed default is VINDICATED for far-tail mission sizing**; the earlier CVaR95-based "sqrt best / cubed past the optimum" was an artifact of too-shallow a metric (walked back). Headline framing: *match the training transform's tail-weight to the sizing percentile* — size at p99 → any mild transform; size at 3σ/CVaR99.9/worst → cubed. log is unambiguously worst at every depth. The dual-role mechanism still holds (cubed = noisy rank-by-worst-of-10) but at the far tail the extreme-compression benefit outweighs the selection-noise cost. fig_cost_transform must plot DV vs sizing-percentile (a family of lines), not a single-percentile bar. The campaign stays cubed (correct for far-tail sizing AND the cross-study-consistent regime). **CURATION SHAPING (Study C-sub / 06), far-tail n=10000 — the SAME worst-case-knob story, even cleaner: bucket=max (the default) DOMINATES the far tail by a WIDENING margin.** Bucket = which seed within each cost-CDF quantile bin trains the policy (min=easiest … max=hardest). Far-tail (n=10000): max CVaR99 144.6 / CVaR99.9 153.0 / max 160.1; random 155.8 / 173.1 / 189.6; middle 152.6 / 193.9 / 236.5; **min 176.1 / 225.9 / 245.2 (catastrophic)**. min has the BEST mean (117.8) but blows up the extreme tail — the textbook "optimize the average, blow up the worst case" failure, the perfect illustration for the sizing argument. middle is bimodal (decent p99, terrible extreme, like squared). So **bucket=max is decisively vindicated for far-tail sizing** (no tradeoff at the design depth, unlike the shallow-CVaR95 pre-fix "middle best mean / max best worst-case" read). TRIM is refuted again (n=1000: trim_0=max CVaR95 137.6 < trim_10 142.0 < trim_20 145.3 — trimming the extreme deciles monotonically hurts the tail; the hard decile builds robustness). **UNIFIED NARRATIVE: cubed (transform) and max (bucket) are the SAME mechanism** — both are worst-case-leaning training knobs that compress the extreme tail by forcing the policy to handle hard cases; both win the far tail (the sizing metric) while shallow metrics favor milder choices. Quartet legs 3+4 are one idea: *worst-case-leaning training (tail-weighted objective + worst-case curation) minimizes the propellant-sizing design-case DV.*
- [ ] 8.5 Sample efficiency: training_n_sims (fig_training_n_sims). **RESULT (08):** view A (rotating, fixed gens) sweet spot n_sims=10 (n=2 too noisy, n>=20 wasted compute); view B (adaptive, allocation) **n_sims=2 @ 10000 gens DOMINATES (109.9 mean / CVaR95 117.5 / far-tail CVaR99 123.7, ~10 m/s < the n=10 anchor, far beyond sigma_run)**. CAVEAT: 1.8x actual sims (21M vs 11.6M -- validation/curation scale with n_gen; quote actual sims), but 2x compute ~2 m/s (budget axis) so ~8 m/s is genuine ALLOCATION gain. Mechanism (unifies A+B, deepens Study C): few-sims noise is a tradeoff bought out by MORE generations (more non-stationary diversity + selection steps) -- many-gens-few-sims beats balanced, like small-batch many-step SGD. **SUPERSEDED as the headline (chain: adaptive_2 3998@n=2/10000 → dense_515 @n=2/20000 → Mamba_962 @n=2/20000, the 10c sizing winner; see 8.6 / the headline).** Each step improved the sizing tail; the final lever was ARCHITECTURE (recurrent state), not the allocation. The allocation lesson (n_sims=2 best, ~2x slower than n_sims=5) still stands and is the reason the deployed headline + the arch sweep use a small-sims/many-gens allocation. The no-plateau→plateau characterization (both the 515- and 972-nets kept improving for ~15-20k gens then plateaued) is a planned methodology figure: the non-stationary objective makes training compute-bound, not overfitting-bound.
- [ ] 8.6 Architecture Pareto + capability floor (fig_pareto): dense-vs-recurrent + where guiding collapses. **RESULTS so far:** (a) Capability floor (09, dense, n_sims=10): NO collapse even at 102 params (100% capture, 120.8 mean); sweet spot ~515 BEATS 3998 (117.4/128.7 vs 120.4/137.6) → 3998 is over-parameterized. (b) **GA-dimensionality head-to-head (515 vs 972, SAME allocation n=2/20000 gens, trained to plateau):** 515 plateaus LOWER (val RMS 1.326e6) and deploys BETTER (109.7 / CVaR95 117.0 / CVaR99.9 128.1) than 972 (1.433e6; 112.4 / 121.6 / 130.7) — more params plateau worse AND deploy worse (2.6-4.6 m/s on bulk-and-tail; only the single max is worse). → **more parameters HURT under GA** (gradient-free → higher-dim search is harder; the "wide nets learn faster/easier" gradient-descent intuition does NOT transfer). The 6-family sweep (10, pending) tests whether this is architecture-independent. **Sweep settings:** `param_sweep --train` now accepts `--training-n-sims` (commit cbc4792); run at `--training-n-sims 5 --n-gen 8000 --n-pop 300` (n_sims=5 = efficient allocation; n_sims=2 reserved for the deployed headline, prohibitive at 24 cells; ~8000 gens plateaus ~500-2000, the ~4000 cells won't fully plateau — footnote). Budget points 500/1000/2000 are the cross-arch core (recurrent archs floor ~450; sub-500 is dense-only/09); 4000 optional. fig_pareto must carry param counts as DATA (manifest), not name-regex (the floor anchors live under paper/).
- [ ] 8.7 Output parameterization (fig_output_param): atan2(2D) wins (117.4 mean / 128.7 CVaR95) over delta (119.9/141.6) and scaledpi (122.2/140.4) — but on EQUAL footing (all GA) the pre-fix ~25 m/s "crush" shrinks to ~3-5 m/s mean; the real, durable advantage is on the TAIL (~12 m/s CVaR95), which is the sizing metric. State that the pre-fix gap was a different-optimizer artifact (atan2 was islands, the 1D heads PSO).
- [ ] 8.8 Input ablation on the headline model (fig_ablation) + input-report saturation stats.
- [ ] 8.9 Classical vs NN — the **3-way NN / joint-FTC / FNPAG** (fig_classical_vs_nn compute-vs-CVaR99.9 scatter). **RESHAPED by Study E (07):** joint-reference optimization recovered FTC from 170.7 to **126.2 mean / 142.9 CVaR95 (−44 m/s, paired p=3e-165)** — confirming the reference WAS FTC's weakness. So the best classical is now **joint-FTC**, which MATCHES FNPAG (124.3 / 144.0) on accuracy across the whole distribution (jointftc_vs_fnpag tied: FNPAG +2 m/s mean, joint-FTC better CVaR95) AND is ANALYTIC/fast (~50× cheaper than FNPAG's predictor). The NN beats this much stronger bar — and the **Mamba_962 headline (10c sizing winner)**: `nn_vs_jointftc` **dMean −16.4 / dCVaR95 −27.6** (paired, 99.9% win) and, at the sizing depth (far-tail n=10000), **NN CVaR99.9 124.5 vs joint-FTC ~164 ≈ FNPAG ~165 — ~40 m/s better at the design tail** (the recurrent NN compresses the extreme tail far better than either classical). Headline: *a recurrent NN (Mamba_962) beats the best classical, and the margin GROWS at the sizing tail; near-FTC compute (23× faster than FNPAG).* Secondary classical story: joint-FTC = FNPAG accuracy at ~68× less compute (5b: NN-mamba 3.68 / NN-dense 2.40 / FTC 1.25 / FNPAG 86.1 ms/sim). NN still crushes fixed-FTC/pred_guid. EC/eqglide/piecewise to an appendix table. 5b/5c DONE; σ_run on the tail comes from 10c (exp-11 obsolete). HONEST CAVEAT (5c): off-nominal, the analytic joint-FTC is MORE robust (capture drop 5.5% vs NN 9.9%) — NN wins the NOMINAL sizing tail. Aggregator `nn_vs_*` + fresh-pool already re-pointed to `headline/mamba_p962`.
...
REWRITTEN from old Task 17, sharpened 2026-06-14: the lead discussion point is **the matched-system result** — the adaptive-seed methodology is what makes GA usable (GA is the *worst* optimizer under fixed seeds, the *best* under adaptive; Study C, the 42 m/s swing). Without the methodology you would deploy CMA-ES (best-under-fixed); with it, GA dominates — so the methodology, not the optimizer, is the contribution. Then: robustness via p99/CVaR95 with σ_run honesty; the headline-at-3998 vs floor-at-~200-400 parameter-efficiency tension; why dense + engineered autoregressive inputs (conditional on the GA-regime sweep outcome); cubed/max framed via propellant sizing (the tail sizes the ergols; a ~1-2 m/s mean concession buys the design-case budget down) with the measured paired deltas quoted either way; the optimizer-matters-only-for-NN finding (8.2); and the **deployability triangle** (NN = FNPAG accuracy at FTC speed). Conclusion: **the 2009 GA endures — but only because the moving-environment training methodology unlocks it**; the contribution is that methodology + the dimensionality/floor maps + the predictor-corrector-parity-at-flight-deployable-cost result; future work (skip-entry, Earth-return, online adaptation; the log-train/tail-select hybrid from Study D). Echo the 2009 closer, answered.
