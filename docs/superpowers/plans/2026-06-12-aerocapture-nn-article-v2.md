...
- [ ] 8.9 Classical vs NN — the **3-way NN / joint-FTC / FNPAG** (fig_classical_vs_nn compute-vs-CVaR99.9 scatter). **RESHAPED by Study E (07):** joint-reference optimization recovered FTC from 170.7 to **126.2 mean / 142.9 CVaR95 (−44 m/s, paired p=3e-165)** — confirming the reference WAS FTC's weakness. So the best classical is now **joint-FTC**, which MATCHES FNPAG (124.3 / 144.0) on accuracy across the whole distribution (jointftc_vs_fnpag tied: FNPAG +2 m/s mean, joint-FTC better CVaR95) AND is ANALYTIC/fast (~50× cheaper than FNPAG's predictor). The NN beats this much stronger bar — and the **515-net headline (109.7 mean / CVaR99.9 128.1)** widens it well beyond the old ga_300 candidate: **NN ~15 m/s better on the mean than joint-FTC (126.2)** and, at the sizing depth (far-tail n=10000), **NN CVaR99.9 128.1 vs joint-FTC 164.0 ≈ FNPAG 165.0 — ~36 m/s better at the design tail** (the tiny NN compresses the extreme tail far better than either classical). Headline: *a 515-parameter NN beats the best classical at the fastest compute class, and the margin GROWS at the sizing tail.* Secondary classical story: joint-FTC = FNPAG accuracy at ~68× less compute (5b: NN 3.21 / FTC 1.29 / FNPAG 87.0 ms/sim). NN still crushes fixed-FTC/pred_guid. EC/eqglide/piecewise to an appendix table. 5b DONE (idle-box); σ_run (11) calibrates the NN-vs-classical gap (large enough it is not at risk). **Re-point the aggregator `nn_vs_*` paired tables + fresh-pool re-quote to `dense_p515_ga_paper_best` (currently ga_300).**
