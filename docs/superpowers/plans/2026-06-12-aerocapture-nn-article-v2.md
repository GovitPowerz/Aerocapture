...
- [ ] 8.4 Objective shaping: cost_transform (fig_cost_transform) + curation bucket/trim (fig_curation), with the DUAL-ROLE interpretation: the per-sim transform sets (a) which moment the seed-RMS optimizes (cubed = tail = the sizing objective) AND (b) the variance of the fitness estimate at n_sims=10 (cubed ~ rank-by-worst-of-10 -> noisy selection, slower convergence; the optimizers are all rank-based, so a post-aggregation monotonic transform would be a no-op -- the effect is estimator noise, not gradient conditioning). Decision rule on the paired `cubed_vs_log` table: if cubed beats log on CVaR95, the tail-weighting pays for its noise cost -- report as a win; if they tie on CVaR95 while log wins the mean (the pre-fix pattern: 137.3 vs 137.9 CVaR, log -1.99 m/s mean), report that the convergence cost cancels the tail gain at this n_sims, note the bucket=max interaction (worst-per-bin + cubed concentrates fitness on ~1-2 seeds), and flag the log-train/tail-select hybrid (train under log for clean rankings, promote on a tail-weighted validation metric) as the identified follow-up -- a one-knob change (`validation_cost_transform`), deliberately NOT run in this campaign.
