...
`[cost_function]` is read only by the Python training pipeline, not by Rust serde (`[network]` was too when this was written; it has since become the Rust-owned `TomlNetwork`). `TomlConfig` does not declare these fields, but since it doesn't use `deny_unknown_fields`, serde silently ignores them. This is intentional — these sections pass through Rust parsing without issue. Since #105 the pass-through is root-level only: every non-flatten section struct denies unknown keys, and the Python-only keys inside Rust-owned sections (`[guidance.neural_network] scaffolding` / `warm_start_from`, `[network] layer_sizes` / `activations` / `qat_*`, `[guidance.piecewise_constant] reference_only`) are declared on the mirror with a `// Python-only` comment.
