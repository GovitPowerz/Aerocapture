...
`[cost_function]` is read only by the Python training pipeline, not by Rust serde (`[network]` was too when this was written; it has since become the Rust-owned `TomlNetwork`). When this was written `TomlConfig` did not declare these sections and, without `deny_unknown_fields`, serde silently ignored them — the pass-through was intentional. Since #105 (ADR-0008) the root denies unknown section names and the seven Python-only sections (`[optimizer]`, `[cost_function]`, `[warm_start]`, `[rl]`, `[checkpoints]`, `[reference]`, `[corridor]`) are declared on `TomlConfig` as pass-through `toml::Table`s; every section struct denies unknown keys, and the Python-only keys inside Rust-owned sections (`[guidance.neural_network] scaffolding` / `warm_start_from`, `[network] layer_sizes` / `activations` / `qat_*`, `[guidance.piecewise_constant] reference_only`) are declared on the mirror with a `// Python-only` comment.
