...
def main():
    ...
    for i, (label, color, prefix) in enumerate(OPTS):
        ...
        for ax in (ax_t, ax_b):
            ax.bar(offs, vals, width, color=color, label=label if ax is ax_b else None, edgecolor="white", linewidth=0.4, zorder=3)
        ...
        for xo, v in zip(offs, vals, strict=True):
            ...
            tgt.annotate(f"{v:.0f}", (xo, min(v, UP_HI if tgt is ax_t else LOW_HI) + 0.4), ha="center", va="bottom", fontsize=6.0, color=color, rotation=90)
    ...
    ax_b.legend(ncol=6, fontsize=7.5, loc="upper center", columnspacing=1.0, handlelength=1.2)
