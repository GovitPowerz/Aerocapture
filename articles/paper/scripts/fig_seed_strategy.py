...
def main():
    ...
    br = ax.bar(
        x + w / 2, rot_cvar, w, color=ROTATING, label="rotating seeds", zorder=2, hatch="//", edgecolor="white", linewidth=0.4
    )  # hatched so the pair is not encoded by red/green alone
    ...
    xdots = {("ga", "fixed"): x[0] - w / 2, ("ga", "rotating"): x[0] + w / 2, ("cmaes", "fixed"): x[3] - w / 2, ("cmaes", "rotating"): x[3] + w / 2}
    for (stem, strat), xi in xdots.items():
        vals = [groups[f"{stem}_{strat}"][s]["dv_cvar95"] for s in ("s1", "s2", "s3")]
        ax.scatter([xi] * 3, vals, s=17, color="#111111", zorder=5, alpha=0.85, label="independent seeds" if (stem, strat) == ("ga", "fixed") else None)
    ...
    ax.annotate(
        "",
        xy=(ga_xr, rot_cvar[0] + 6),
        xytext=(ga_xf + 0.10, fixed_cvar[0] + 6),
        arrowprops={"arrowstyle": "->", "color": "#b8860b", "lw": 2.2, "connectionstyle": "arc3,rad=-0.35"},
        zorder=6,
    )
    ...
    ax.set_title("Study C: GA needs non-stationary seeds; CMA-ES is flat (bars = CVaR$_{95}$, black tick = mean)", fontsize=9.5, loc="left")
