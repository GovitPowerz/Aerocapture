"""Parse Fortran final.* Monte Carlo final conditions into DataFrames."""
...
def parse_final(filepath: str | Path) -> pd.DataFrame:
    """Parse a final.* MC results file into a DataFrame.

    Each line is one simulation run: integer sim number + D-notation floats.
    Format: 1x,i5,52(1x,d15.7)

    Args:
        filepath: Path to the final.* file.

    Returns:
        DataFrame with columns for the final conditions data.
    """
