...
def test_gru_ppo_smoke_5_updates(tmp_path: Path) -> None:
    ...
    # Load the full PPO-GRU config with base inheritance resolved, then shrink
    # every dimension so the smoke test fits in CI (~60-90s).
    resolved = load_toml_with_bases(Path("configs/training/msr_aller_gru_ppo_train.toml"))

    # Shrink the architecture: Dense(21->8) -> GRU(8, hidden=8) -> Dense(8->2).
    ...
    ppo_section["bptt_length"] = 16  # 64 / 16 = 4 chunks
    ...
    output_dir = tmp_path / "neural_network_gru_ppo_smoke"
    output_dir.mkdir()
