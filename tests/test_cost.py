...
class TestDvCost:
    ...
    def test_quadratic_above_threshold(self) -> None:
        dv = np.array([10000.0])
        result = dv_cost(dv, threshold=1000.0)[0]
        x = 9000.0
        expected = 1000.0 + x + x**2 / 20000.0
        assert abs(result - expected) < 1e-6

    def test_c0_continuity_at_threshold(self) -> None:
        t = 1000.0
        below = dv_cost(np.array([t - 1e-10]), threshold=t)[0]
        above = dv_cost(np.array([t + 1e-10]), threshold=t)[0]
    ...
    def test_strong_gradient_at_non_capture(self) -> None:
        """Slope at dv=10000 should be ~1.9, much stronger than log_cap's 0.1."""
        eps = 1.0
    ...
    def test_wide_cost_spread_for_non_captures(self) -> None:
        """Non-capture range should span >10000 cost (vs ~700 for log_cap)."""
        barely_hyper = dv_cost(np.array([10000.0]))[0]
        early_crash = dv_cost(np.array([20000.0]))[0]
        assert early_crash - barely_hyper > 10000
...
class TestUnifiedComputeCost:
    ...
    def test_non_capture_has_high_cost(self) -> None:
        ...
        cost = compute_cost(final)
        # dv_cost(10000) = 1000 + 9000 + 9000^2/20000 = 14050
        assert 14000 < cost < 14200

    def test_crash_dv_produces_very_high_cost(self) -> None:
        final = self._make_final(5, dv=20000.0, g=5.0, q=50.0)
        cost = compute_cost(final)
        # dv_cost(20000) = 1000 + 19000 + 19000^2/20000 = 38050
        assert 38000 < cost < 38200
    ...
    def test_custom_dv_threshold(self) -> None:
        """Lower threshold means quadratic kicks in earlier, so cost is higher."""
