...
class TestProblemShape:
    def test_n_var(self):
        p = AerocaptureProblem(
            param_specs=_make_specs(), toml_path="dummy.toml", seeds=[42],
            cost_kwargs={}, scheme="equilibrium_glide",
        )
        assert p.n_var == 3
...
class TestProblemEvaluation:
    def test_evaluate_returns_correct_shape(self):
        specs = _make_specs()
        p = AerocaptureProblem(
            param_specs=specs, toml_path="dummy.toml", seeds=[42],
            cost_kwargs={"dv_threshold": 1000.0, "g_load_limit": 15.0, "heat_flux_limit": 200.0,
                         "heat_load_limit": 25000.0, "g_load_weight": 1000.0, "heat_flux_weight": 1000.0,
...
class TestBuildOverrides:
    ...
    def test_n_sims_override(self):
        p = AerocaptureProblem(
            param_specs=_make_specs(), toml_path="dummy.toml", seeds=[42],
            cost_kwargs={}, scheme="equilibrium_glide", n_sims_override=50,
        )
