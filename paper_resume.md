...
> **⚠ SUPERSEDED ON THE SIZING TAIL (10c, 2026-06-25): Mamba_962 beats this dense net beyond σ_run** (CVaR99.9 124.5 vs 139.2; max non-overlapping). dense_515's tail looked good only because its s1 was a lucky draw. The deployed headline should become **Mamba_962** (`training_output/mamba_p962_long/`) on the sizing metric, with dense_515 kept as the efficiency reference. See pending item 6. The dense-515 finalization below (bundle/aggregator/ablation) is real but now describes the EFFICIENCY reference, not the sizing headline.
...
6. ~~Run 10c (tail σ_run)~~ **DONE — VERDICT: Mamba_962 beats Dense_515 on the sizing tail BEYOND σ_run. The deployed headline should become Mamba_962.** s2/s3 fresh runs (`training_output/paper/tail_repeats/{dense515,mamba962}_s{2,3}`), all 100% capture at n=10000. σ_run over {s1,s2,s3} (far-tail CVaR99.9 mean / range): **DENSE 139.2 / 21.5** (s1 128.1 was the LUCKY draw — typical ~140!) vs **MAMBA 124.5 / 9.3**. max: DENSE 159.0 / 38.3 (s3 hit **184**) vs MAMBA 127.6 / 15.2. Overlap test deepening into the tail: CVaR95 ΔMean −5.6 (overlap), CVaR99.9 −14.7 (barely overlap), **max −31.4 NON-OVERLAPPING (mamba's worst run 136.9 < dense's best 146.1)**. So (a) my 10b quote used dense's favorable s1 — the REAL gap is ~−15 CVaR99.9, not −6; (b) Mamba is also ~2× more CONSISTENT (half the σ_run) — the dense net's extreme tail is a high-variance crapshoot, mamba's state gives robust tail behavior. The advantage GROWS deeper into the tail (where tanks are sized). Headline → **Mamba_962** (sizing); dense_515 = efficiency reference (½ params, memoryless). **NEXT (user chose):** run `10c_tail_sigma_repeats.sh` again — the LSTM line is now uncommented, so it trains ONLY lstm1082_s2/s3 (dense/mamba skip via final_selection); lstm s1 = `lstm_p1082_long` (CVaR99.9 123.2). Decides mamba-vs-lstm on equal σ_run footing BEFORE locking the headline. THEN re-point bundle/aggregator/ablation/fresh-pool to the winner (the finalization, like was done for dense_515).
7. **Run 11 (seed-repeats)** for σ_run on the optimizer/dimensionality cells (n=10/2000 regime) — distinct from 10c's headline-allocation tail repeats.
