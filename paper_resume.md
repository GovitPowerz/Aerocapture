...
> on top of main `e9c73d2` (PR #57), NOT pushed. Stack, oldest first: `6d0cb10` second full review
> (15 findings: stats.json gitignore build blocker, 924 requote, far-tail figures, SIZE_HALF
> legibility, qdyn claim removed, sigma_run tail_groups) · `6bfd095` appendix reproduction check
> (stats bit-exact; 2 nn_dense panels re-rendered on branch code) · `a199242` retitle "Seventeen
> years later" + abstract caveat resolution · `b5426b7` external reviews 1-3 (Table 3 Viol.% column +
> joint rows; **LSTM best seed heat-load-INFEASIBLE 15.6%/14.4% — Mamba is the only in-envelope tail
> winner**; bootstrap CIs + viol rates in `far_tail_eval.py` incl. `label:toml:bundle_key` weight
> pinning after the dense_p515 training-dir drift 140.3-vs-128.1; **loss-vs-tail Spearman 0.91 →
> §6.3 refined to "loss does not pick the tail winner"** + `fig_loss_vs_tail`; abstract trimmed;
> Appendix reproduction details; Table 4 CI column) · `b56b5ee` hard page breaks · `f04b76d` MIT
> license · `7339020` IMPROVEMENTS.md future work · `a971051` probes appendix (CfC/xLSTM/Mamba-3
> negative result; probe data in `data/probes/`; probe_common records viol rates — all 9 arms 0%) ·
> `2e0a507` Lahoti et al. 2026 citation · `eff220e` **Mamba-3 arXiv restyle** (Libertinus, ~6.5in
> block, booktabs captions-above, harvard-cite-them-right "References", crimson cites/blue refs) ·
> `7a457a1` Table 4 column-count fix (7 cells in a 6-column grid — shipped 3 commits; verify tables
> structurally AND on final-state renders) · `7332a41` appendix reorder **A=repro B=probes C=cards**
> + de-tooling (NO CLI/paths/module names in paper text; language/library names OK) + card capture
> row last · `e381ac6` card constraint panels compact-native (readable at 3-across) · `5745050`
> Appendix A systems paragraphs (simulator/bit-validation record, harness ~2e7 sims ≈ 20 core-hours,
> one-runtime + 1e-16 cross-language parity). **Conventions:** compile from repo root
> (`typst compile articles/paper/paper.typ articles/paper/paper.pdf`, 48 pp); refresh
> `~/Desktop/aerocapture_paper_2026-07-08.pdf` after changes; page-proof via
> `typst compile --format png --pages N` + view; every quoted number must be backed by
> `articles/paper/data/` and generated (not transcribed) from it; verify reviewer claims against
> data before acting (reviews 1-3: the LaTeX-artifact items were PDF-extraction ghosts; the
> feasibility item was the real one); reviews 1-3 live in untracked `paper_reviews.md` (DONE);
> arXiv: Typst PDF-only submission is fine (no TeX source requirement for non-TeX PDFs).
