...
6. **Run 10c (tail σ_run) — NEXT, decides the headline.** `experiments/paper/10c_tail_sigma_repeats.sh`: fresh s2/s3 of **dense_515 + mamba_962** at the headline allocation (n=2/512/20000, `--seed 2/3`, eval pool fixed → clean σ_run), resumable. After: far-tail each, quote CVaR99.9 mean±range over {s1,s2,s3}. If mamba's −6 m/s tail edge > σ_run, **the deployed headline becomes Mamba_962 for the sizing tail (dense_515 the efficiency alt)** and the arch story flips from "dense dominates" to "engineered inputs flatten the bulk; internal state wins the sizing tail". (s1 = 10b/headline; mamba s1 sweep-seeded but plateaued; lstm_1082 commented-optional.)
