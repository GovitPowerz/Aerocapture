...
5. **Run 10b (long recurrent challengers) — NEXT, the reviewer-defense for "dense is best".** `experiments/paper/10b_arch_long_challengers.sh` extends the best recurrent sweep cells (lstm_p1082, mamba_p962, gru_p1014) to the HEADLINE depth — n_sims=2, **n_pop=512**, 20000 gens — by resuming each sweep cell's g05000 checkpoint into a `<arch>_p<N>_long` dir via `--output-dir` (+15000 gens; sweep Pareto cells untouched; report ON so each gets a 2M-pool final_eval.parquet directly paired-comparable to the headline). Ordered strongest-first (lstm > mamba > gru); early-stop after lstm+mamba if they settle it. Prediction: land ~108–113, competitive but not beating the 515-dense, and ≥2× its params → dense still wins on efficiency. Each is a multi-day run.
6. **Run 11 (seed-repeats)** for σ_run — add a repeat of the 515-net (at n_pop=512) to confirm the parameter-efficiency + allocation gains are beyond σ_run.
7. Then Phase 2/3 of plan v2: aggregate → figures (Task 3) → Typst (Tasks 6-14) → smart-commit.
