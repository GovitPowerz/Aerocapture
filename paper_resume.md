...
> **As of:** 2026-06-25. **Phase:** ALL campaign experiments DONE (00-10c + 5b/5c). **DEPLOYED SIZING HEADLINE = Mamba_962** (`training_output/mamba_p962_long/`); dense_515 = efficiency reference. 10c verdict: recurrent (Mamba/LSTM) beat dense on the mission-SIZING TAIL beyond σ_run — see "Headline" + items 5-7. exp-11 OBSOLETE/skipped. Bundle + aggregator + all diagnostics point at Mamba. Next = figures + Typst (Phase 2/3 of plan v2). No training/eval left.
...
**`training_output/mamba_p962_long/` — 962-param net (Dense→Mamba SSM→Dense), GA + adaptive + cubed + max, n_sims=2, n_pop=512, 20000 gens. Config: `configs/training/sweep/mamba_p962.toml`.**
...
| 10b | `10b_arch_long_challengers.sh` | best recurrent cells → 20000 gens | DONE (recurrent beat dense on the tail) |
