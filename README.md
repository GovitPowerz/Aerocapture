...
3. **Lateral guidance** — Roll sign management via predictive first-order inclination projection (projects error forward by configurable tau seconds; reverses only when projected error exceeds threshold). Shared by unsigned-magnitude schemes (EqGlide, EnergyCtrl, PredGuid, FNPAG). Remains active during exit phase for inclination correction. NN and PiecewiseConstant produce signed bank angles and bypass both lateral and exit guidance entirely.
