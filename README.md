...
| PPO baseline | NN — dense, 515 params, PPO warm-started from the efficiency reference | per-scenario | 10⁶ | 100.00 | 0.02 | 235.2 ± 2.4 | 403 | 1.88 |
...
- **Policy gradients do not compete under the matched protocol** (paper Section 5, issue #101). PPO from scratch captures but pays 2–3x the correction ΔV of the population-trained cell of the same architecture and leans on the heat-flux limit (5.6% violations, 53% for the GRU cell); PPO warm-started from a champion deploys the champion (its best validation checkpoint is the starting point) and then drifts off it. Raw data: `rl/*` keys in `articles/paper/data/results.json` (2M pool, n = 1000, paired) and `experiments/ou_marginal/confirmatory_marginal.json` (10⁶).
